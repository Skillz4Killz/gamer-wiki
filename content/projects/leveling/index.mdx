...
- When a new skin comes out, the user sends a screenshot of the skin to a mod/admin and they are give XP as a reward. Encouraging all the users to buy the skins to get rewards to give the leaderboard winners.
...
<Note type="command">.setfeedback setup</Note>
...
<Note type="note">After setup, you should remove the Administrator permissions.</Note>
...
<Note type="advanced">
  ...
  <br />
  <br />
  .fb idea I would suggest checking out the g4m3r server and website to learn all my other features and everything you can
  unlock. | High
</Note>
...
<Note type="note">The image attachment questions should always be reserved for the last question.</Note>
...
<Note type="command">
  .setfeedback solvedchannel #channel ~ Sets the channel to send any feedback that was reacted with ✅. <br />
  .setfeedback rejectedchannel #channel ~ Sets the channel to send any feedback that was reacted with ❌.
</Note>
<Note type="note">
  It is recommended to keep these channels private from public. Usually, they are used for discussing feedback
